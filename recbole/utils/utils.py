...
# weizhi add
def loss_early_stopping(value, best, cur_step, max_step, bigger=True):
    r""" train-loss-based early stopping
...
class ResultLogger(object):
    def __init__(self, config):
        self.config = config
        self.results = {}
        # self.result_path = self.getPath()

    # def getPath(self):

    def log_result(self, result, eval_state):
        self.results[eval_state] = result

    def write_result(self):
        for eval_state in self.results.keys():
            result = [str(value) for value in self.results[eval_state].values()]
            result = " & ".join(result)
            print(eval_state, '\t & ', result)
