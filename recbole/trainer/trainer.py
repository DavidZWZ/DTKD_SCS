...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.epochs_con = config['epochs_con'] 
        self.epochs_cf = config['epochs_cf'] 
        self.epochs_kd_emb = config['epochs_kd_emb'] 
        self.epochs_kd_sf = config['epochs_kd_sf'] 
        self.epochs_kd_score = config['epochs_kd_score']
        ...
        self.saved_model_file = os.path.join(self.checkpoint_dir, self.config['model'], saved_model_file)
    ...
    def _valid_epoch(self, valid_data, validate_mode=None, show_progress=False):
        ...
        """
        if validate_mode == 'cs':
            valid_result = self.evaluate_cs(valid_data, 0, load_best_model=False, show_progress=show_progress)
        elif validate_mode == 'rec':
    ...
    def load_tea_checkpoint(self, tea_file):
        r"""Load the model parameters information and training information.
        ...
        """
        tea_file = str(tea_file)
        checkpoint = torch.load(tea_file)
        ...
        message_output = 'Teacher model {} loaded'.format(tea_file)
        self.logger.info(message_output)
    ...
    def fit(self, train_data, valid_data=None, validate_mode='rec', teacher_type=None, load_model_file=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        """
        if load_model_file:
            self.load_tea_checkpoint(load_model_file)
        else:
            ...
            if teacher_type:
                if teacher_type == "con":
                    self.epochs = self.epochs_con
                elif teacher_type == "cf":
                    self.epochs = self.epochs_cf
            for epoch_idx in range(self.start_epoch, self.epochs):
                ...
                if (epoch_idx + 1) % self.eval_step == 0:
                    ...
                    valid_score, valid_result = self._valid_epoch(valid_data, validate_mode=validate_mode, show_progress=show_progress)
    ...
    def _distill_epoch(self, train_data, teacher_con, teacher_cf, epoch_idx, distill_mode='sf', show_progress=False):
        r"""Train the model in an epoch

        Args:
            train_data (DataLoader): The train data.
            epoch_idx (int): The current epoch id.
            distill_mode: "sf" represents soft score, "emb" represents latent space, score is the rec score
            show_progress (bool): Show the progress of training epoch. Defaults to ``False``.
        ...
        teacher_con.model.eval()
        teacher_cf.model.eval()

        if distill_mode == 'sf':
            self.model.train()
            soft_con = teacher_con.model.calculate_soft
            soft_cf = teacher_cf.model.calculate_soft
        elif distill_mode == 'emb':
            self.model.train()
            embed_con = teacher_con.model.calculate_embed
            embed_cf = teacher_cf.model.calculate_embed
        elif distill_mode == 'score':
            self.model.train()
            embed_cf = teacher_cf.model.calculate_embed      
        total_loss = None
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            self.optimizer.zero_grad()
            if distill_mode == 'sf':
                losses = self.model.distill_soft(interaction, soft_con, soft_cf)
            elif distill_mode == 'emb':
                losses = self.model.distill_embed(interaction, embed_con, embed_cf)
            elif distill_mode == 'score':
                losses = self.model.distill_score(interaction, embed_cf)  
            if isinstance(losses, tuple):
                loss = sum(losses)
                loss_tuple = tuple(per_loss.item() for per_loss in losses)
                total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
    ...
    def kd(self, train_data, valid_data, tearcher_con, tearcher_cf, validate_mode='cs', distill_mode='sf', verbose=True, saved=True, show_progress=False, callback_fn=None):
        r"""distill the teacher model based on the train data and the valid data.
        ...
        valid_step = 0
        if distill_mode == 'sf':
            epochs = self.epochs_kd_sf
        elif distill_mode == 'emb':
            epochs = self.epochs_kd_emb
        elif distill_mode == 'score':
            epochs = self.epochs_kd_score
        for epoch_idx in range(self.start_epoch, epochs):
            # train
            training_start_time = time()
            train_loss = self._distill_epoch(train_data, tearcher_con, tearcher_cf, epoch_idx, distill_mode=distill_mode, show_progress=show_progress)
            self.train_loss_dict[epoch_idx] = sum(train_loss) if isinstance(train_loss, tuple) else train_loss
            training_end_time = time()
            train_loss_output = \
                self._generate_train_loss_output(epoch_idx, training_start_time, training_end_time, train_loss)
    ...
    def _full_sort_batch_eval_cs(self, batched_data, eval_mode):
        interaction, history_index, positive_u, positive_i = batched_data

        positive_i_pos = [self.item_id_pos_map[int(id)] for id in positive_i]
        positive_i_pos = torch.LongTensor(positive_i_pos)
        try:
            # Note: interaction without item ids
            scores = self.model.full_sort_predict(interaction.to(self.device))
        except NotImplementedError:
            inter_len = len(interaction)
            new_inter = interaction.to(self.device).repeat_interleave(self.cs_item_num)
            batch_size = len(new_inter)
            new_inter.update(self.item_tensor.repeat(inter_len))
        ...
        if scores.size(0)%self.cs_item_num==0:
            scores = scores.view(-1, self.cs_item_num)
        else:
            scores = scores.view(-1, self.tot_item_num)
            scores = scores[:,self.cs_item_id]
        scores[:, 0] = -np.inf
        if history_index is not None and eval_mode in (0,1,2,3):
            history_index_i = [self.item_id_pos_map[int(id)] for id in history_index[1]]
            history_index_i=(torch.LongTensor(history_index_i))
            history_index_mapped = ([history_index[0],history_index_i])
            scores[history_index_mapped] = -np.inf
        return interaction, scores, positive_u, positive_i_pos


    def _neg_sample_batch_eval_cs(self, batched_data, eval_mode):
    ...
    @torch.no_grad()
    def evaluate_cs(self, eval_data, eval_mode, load_best_model=True, model_file=None, show_progress=False):
        r"""Evaluate the model based on the eval cold start data.
        ...
        #init item_id_pos mapping and item_id
        dataset_eval = eval_data.dataset
        self.item_id_pos_map = dataset_eval.item_id_pos_map
        self.cs_item_id = list(dataset_eval.item_id_pos_map.keys())  
        if isinstance(eval_data, FullSortEvalDataLoader):
            eval_func = self._full_sort_batch_eval_cs
            # if self.item_tensor is None: chage every time
            self.item_tensor = eval_data.dataset.item_feat_cs.to(self.device)
        else:
            eval_func = self._neg_sample_batch_eval_cs
        if self.config['eval_type'] == EvaluatorType.RANKING:
            self.tot_item_num = eval_data.dataset.item_num
            self.cs_item_num = eval_data.dataset.num_cs_item
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            interaction, scores, positive_u, positive_i = eval_func(batched_data, eval_mode)
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
            self.eval_collector.eval_batch_collect_cs(scores, interaction, positive_u, positive_i)
