...
class AbstractSampler(object):
    ...
    def __init__(self, distribution):
        ...
        self.used_ids = self.get_used_ids()  # get_used_ids
    ...
    def sampling(self, sample_num):
        ...
        if self.distribution == 'uniform':
            if self.eval_mode == 'cs':
                return self._uni_sampling_cs(sample_num)
            else:
    ...
    def sample_by_key_ids(self, key_ids, num):
        ...
        if (key_ids == key_ids[0]).all(): # for one user
            key_id = key_ids[0]
...
class Sampler(AbstractSampler):
    ...
    def __init__(self, phases, datasets, distribution='uniform', eval_mode=None, positive_len=None):
        ...
        self.item_ids_list = (datasets[1].item_id, datasets[4].item_id) # 0-train, 1-3 warm item, 4-5 cold item

        super().__init__(distribution=distribution)
        self.used_ids = self.get_used_ids_cs()  # get_used_ids
    ...
    def _uni_sampling_cs(self, sample_num):
        if self.phase == 'train':
            return np.random.randint(1, self.item_num, sample_num)
        elif self.phase in ('valid_ww', 'test_ww', 'test_cw'):
            return np.array(random.sample(self.item_ids_list[0], sample_num))
        elif self.phase in ('test_wc', 'test_cc'):
            return np.array(random.sample(self.item_ids_list[1], sample_num))
        else:
            raise ValueError(
                    'Not supporting sampling for the phase'
                )
    ...
    def get_used_ids_cs(self):
        ...
        for phase, dataset in zip(self.phases, self.datasets):
            ...
            for uid, iid in zip(dataset.inter_feat[self.uid_field].numpy(), dataset.inter_feat[self.iid_field].numpy()):
                cur[uid].add(iid)
            if phase in {'train','valid_ww'}: # include valid test_ww valid_ww
                last = used_item_id[phase] = cur
            else:  # after test_ww
                used_item_id[phase] = cur
                last = [set() for _ in range(self.user_num)]
