...
class Dataset:
    ...
    def __init__(self, config):
        ...
        default_file = os.path.join(self.config['checkpoint_dir'], self.config["dataset"], 'dataloader.pth')
        if not os.path.exists(default_file) or self.config['dataset'] != 'book-crossing':
            self._from_scratch()
    ...
    def _data_filtering(self):
        ...
        self._reset_index()
        self._init_warm_cold_user_item()

    def _init_warm_cold_user_item(self):
        self.item_id = None #candicate item_id in different cs settings
        self.warm_item = None
        self.cold_item = None
        self.num_cs_item = None
        self.num_cs_user = None
        self.item_id_pos_map = {}
        self.item_feat_cs = None
        self.uid_field_cs = None
        self.iid_field_cs = None
    ...
    def _remap(self, remap_list):
        ...
        for (feat, field, ftype), new_ids in zip(remap_list, new_ids_list):
            ...
            if ftype == FeatureType.TOKEN:
                ...
                if str(field) == self.title:
                    for i, feat_i in enumerate(feat[field]):
                        feat[field][i] = " ".join(feat_i)
                    title_embeddings = self.bert.encode(feat[field])
                    for i in range(len(feat[field])):
                        feat[field][i] = title_embeddings[i]
                else:
                    split_point = np.cumsum(feat[field].agg(len))[:-1]
    ...
    def deepcopy(self, new_inter_feat):
        """Given a new interaction feature, return a new :class:`Dataset` object,
        whose interaction feature is updated with ``new_inter_feat``, and all the other attributes the same.
    ...
    def _grouped_index_cs(self, group_by_list):
        index = {}
        for i, key in enumerate(group_by_list):
            if key not in index:
                index[key] = [i]
    ...
    def split_by_cs(self, user_ratios = [0.8,0.2], ratios=[0.9, 0.1], group_by=None):
        """Split interaction records for cold_start settting.
        ...
        Note:
            Other than the first one, each part is rounded down.
        """
        grouped_item_inter_feat_index = self._grouped_index_cs(self.inter_feat[self.iid_field].numpy())       
        n_items = len(grouped_item_inter_feat_index)
        split_ids = self._calcu_split_ids(tot=n_items, ratios=user_ratios)

        # shuffle splitting
        item_id = list(grouped_item_inter_feat_index.keys())
        random.shuffle(item_id)

        # warm_cold_item for full rank evaluation
        self.warm_item = self.get_item_feature_cs(item_id[:split_ids[0]])
        self.cold_item = self.get_item_feature_cs(item_id[split_ids[0]:n_items])

        num_warm_item, num_cold_item = split_ids[0], n_items - split_ids[0]
        warm_item_inter_index, cold_item_inter_index = [], []
        for index in item_id[:split_ids[0]]:
            warm_item_inter_index.extend(grouped_item_inter_feat_index[index])
        for index in item_id[split_ids[0]:n_items]:
            cold_item_inter_index.extend(grouped_item_inter_feat_index[index])
        wc_item_inter_index = [warm_item_inter_index, cold_item_inter_index]
        wc_item_inter = [self.inter_feat[index] for index in wc_item_inter_index]

        #split by user 
        grouped_user_inter_feat_index = self._grouped_index_cs(self.inter_feat[self.uid_field].numpy())       
        n_users = len(grouped_user_inter_feat_index)
        split_ids = self._calcu_split_ids(tot=n_users, ratios=user_ratios)

        # shuffle splitting
        user_id = list(grouped_user_inter_feat_index.keys())
        random.shuffle(user_id)

        # warm_user for model init (lightgcn)
        num_warm_user, num_cold_user = split_ids[0], n_users - split_ids[0]

        warm_user_inter_index, cold_user_inter_index = [], []
        for index in user_id[:split_ids[0]]:
            warm_user_inter_index.extend(grouped_user_inter_feat_index[index])
        for index in user_id[split_ids[0]:n_users]:
            cold_user_inter_index.extend(grouped_user_inter_feat_index[index])

        warm_user_inter_set, cold_user_inter_set = set(warm_user_inter_index), set(cold_user_inter_index)
        ww_inter_index = [index for index in warm_item_inter_index if index in warm_user_inter_set]
        cw_inter_index = [index for index in warm_item_inter_index if index in cold_user_inter_set]
        wc_inter_index = [index for index in cold_item_inter_index if index in warm_user_inter_set]
        cc_inter_index = [index for index in cold_item_inter_index if index in cold_user_inter_set]
        all_cs_user_item_inter = [ww_inter_index, cw_inter_index, wc_inter_index, cc_inter_index]
        next_df = [self.inter_feat[index] for index in all_cs_user_item_inter]

        # split by ratio
        if group_by is None:
            n_inter_ww = len(next_df[0])
            split_ids = self._calcu_split_ids(tot=n_inter_ww, ratios=ratios)
            next_index = [range(start, end) for start, end in zip([0] + split_ids, split_ids + [n_inter_ww])]
        else:
            grouped_inter_feat_index = self._grouped_index(next_df[0][group_by].numpy())
            next_index = [[] for _ in range(len(ratios))]
            for grouped_index in grouped_inter_feat_index:
                tot_cnt = len(grouped_index)
                split_ids = self._calcu_split_ids(tot=tot_cnt, ratios=ratios)
        ...
        next_ww = [next_df[0][index] for index in next_index]
        next_ds = [self.deepcopy(_) for _ in next_ww+next_df[1:]]

        # modify user item number for full rank evaluation
        nums_cs_item = [num_warm_item, num_warm_item, num_warm_item, num_cold_item , num_cold_item]

        # for model init
        next_ds[0].num_cs_user, next_ds[0].num_cs_item = num_warm_user, num_warm_item
        for i, ds in enumerate(next_ds[1:]):
            ds.num_cs_item = nums_cs_item[i]
            # assign id to position mapping  and item feat in each evaluation dataset
            if i<3:
                for j in range(num_warm_item):
                    ds.item_id_pos_map[int(self.warm_item[self.iid_field][j])] = j
                    ds.item_feat_cs = self.item_feat[self.warm_item[self.iid_field]]
                    ds.item_id = set(np.array(self.warm_item['item_id']))
            else:
                for j in range(num_cold_item):
                    ds.item_id_pos_map[int(self.cold_item[self.iid_field][j])] = j
                    ds.item_feat_cs = self.item_feat[self.cold_item[self.iid_field]]
                    ds.item_id = set(np.array(self.cold_item['item_id']))
        return next_ds
    ...
    def build_cs(self):
        """Processing dataset for cold start evaluation according to evaluation setting,
        including Group, Order and Split.
        See :class:`~recbole.config.eval_setting.EvalSetting` for details.
        ...
        if split_mode == 'RS':
            if not isinstance(split_args['RS'], list):
                raise ValueError(f'The value of "RS" [{split_args}] should be a list.')
            if group_by is None or group_by.lower() == 'none':
                datasets = self.split_by_cs(user_ratios = split_args['UR'], ratios = split_args['RS'], group_by=None)
            elif group_by == 'user':
                datasets = self.split_by_cs(user_ratios = split_args['UR'], ratios = split_args['RS'], group_by=self.uid_field)
            else:
                raise NotImplementedError(f'The grouping method [{group_by}] has not been implemented.')
        else:
            raise NotImplementedError(f'The splitting_method [{split_mode}] has not been implemented.')
    ...
    def get_item_feature_cs(self, list_items):
        """
        Returns:
            Interaction: item features for warm or cold item evaluation
        """
        if self.item_feat_cs is None:
            self._check_field('iid_field')
            inter = Interaction({self.iid_field: torch.LongTensor(list_items)})
            inter.sort(by=self.iid_field)
            return inter
        else:
            return self.item_feat_cs



    def _create_sparse_matrix_cs(self, df_feat, source_field, target_field, form='coo', value_field=None):
        ...
        mat = coo_matrix((data, (src, tgt)), shape=(self.num_cs_user, self.num_cs_item))
        if form == 'coo':
            return mat
        elif form == 'csr':
            return mat.tocsr()
    ...
    def inter_matrix_cs(self, form='coo', value_field=None):
        """Get sparse matrix that describe interactions between user_id and item_id.
        ...
        if not self.uid_field or not self.iid_field:
            raise ValueError('dataset does not exist uid/iid, thus can not converted to sparse matrix.')
        return self._create_sparse_matrix_cs(self.inter_feat, self.uid_field, self.iid_field, form, value_field)
    ...
    def _dataframe_to_interaction(self, data):
        ...
        for k in data:
            ...
            if ftype == FeatureType.TOKEN:
                ...
                if str(k) == self.title:
                    seq_data = [torch.FloatTensor(d) for d in value]
