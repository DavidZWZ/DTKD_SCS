...
class NegSampleEvalDataLoader(NegSampleDataLoader):
    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        if self.neg_sample_args['strategy'] in ('by', 'label'):
            ...
            for uid in self.uid_list: # uid in uid list has none
    ...
    def _next_batch_data(self):
        if self.neg_sample_args['strategy'] == 'by':
            ...
            for idx, uid in enumerate(uid_list):
                ...
                idx_list += [idx for i in range(min(self.uid2items_num[uid], self.config['positive_len']) * self.times)]            
        ...
        elif self.neg_sample_args['strategy'] == 'label':
            uid_list = self.uid_list[self.pr:self.pr + self.step]
            data_list = []
            idx_list = []
            positive_u = []
            ...
            for idx, uid in enumerate(uid_list):
                index = self.uid2index[uid]
                # keep the same for neg sampling
                data_cur = self._neg_sampling(self.dataset[index])
                
                item_num = self.uid2items_num[uid]
                if item_num>=self.rank_len:
                    sample_index = np.random.choice(list(range(item_num)), self.rank_len, replace=False)
                    data_cur = data_cur[sample_index]    
                    idx_list += [idx for i in range(self.rank_len * self.times)]
                else:
                    idx_list += [idx for i in range(self.uid2items_num[uid] * self.times)]
                
                data_list.append(data_cur)

                positive_i_index = ((data_cur[self.label_field] == 1).nonzero(as_tuple=True)[0])
                positive_i_cur = data_cur[positive_i_index][self.iid_field]

                positive_u += [idx for i in range(len(positive_i_cur))]
                positive_i = torch.cat((positive_i, positive_i_cur), 0)
            cur_data = cat_interactions(data_list)
            idx_list = torch.from_numpy(np.array(idx_list))
            positive_u = torch.from_numpy(np.array(positive_u))
...
class FullSortEvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        if config['positive_len']:
            self.positive_len = config['positive_len']
        ...
        if not self.is_sequential:
            ...
            last_uid = None
            positive_item = set()
            uid2used_item = sampler.used_ids  # Problem here
    ...
    def _set_user_property(self, uid, used_item, positive_item):
        if uid is None:
            return
        pos_item_num = len(positive_item)
        if self.positive_len and pos_item_num>=self.positive_len:
            positive_item = set(random.sample(positive_item, self.positive_len))
        # history items become the items in training for test ww, None for test cw wc cc
